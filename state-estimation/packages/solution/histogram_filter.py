...
console_handler.setLevel(logging.INFO) # или другой уровень, который вам нужен
...
def pose_estimation(
    R: float,
    baseline: float,
    left_encoder_delta_ticks: float,
    right_encoder_delta_ticks: float,
    resolution: int
    ) -> Tuple[float, float]:
    ...
    Args:
        ...
        left_encoder_delta_ticks:     left wheel rotation (rad)
        right_encoder_delta_ticks:    right wheel rotation (rad)
        resolution: encoder resolution

    Return:
        d:                  d shift
        delta_theta:        theta shift
    """

    
    delta_phi_left = left_encoder_delta_ticks * 2 * np.pi / resolution
    delta_phi_right = right_encoder_delta_ticks * 2 * np.pi / resolution
...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    d_max = grid_spec['d_min'] + (maxids[0] + 0.5) * grid_spec['delta_d']

    R = robot_spec["wheel_radius"]
    baseline = robot_spec["wheel_baseline"]
    resolution = robot_spec["encoder_resolution"]

    est_d, est_phi = pose_estimation(R = R, baseline = baseline, resolution = resolution, 
        left_encoder_delta_ticks = left_encoder_ticks, right_encoder_delta_ticks = right_encoder_ticks)

    v = est_d  # replace this with a function that uses the encoder
    w = est_phi  # replace this with a function that uses the encoder

    # propagate each centroid
    d_t = grid_spec["d"] + v*np.sin(phi_max)
    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                logger.info(f"predict: i = {i}, i_new = {i_new}, j = {j}, j_new = {j_new }")
